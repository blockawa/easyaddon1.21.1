...
public class AutoLoginXin extends Module {
    ...
    private final Timer queueUseTimer = new Timer();

    /* States */
    private boolean login = false;
    private boolean hasJoinedQueue = false;
    private boolean waitingForQueueUse = false;
    ...
    public final Setting<Integer> queueUseDelay = sgGeneral.add(new IntSetting.Builder()
        .name("右键后延迟")
        .description("右键使用指南针后等待多少秒才标记为已加入队列")
        .defaultValue(1)
        .min(0).max(5).sliderMin(0).sliderMax(5)
        .build()
    );
    ...
    public void onTick(TickEvent.Pre event) {
        ...
        /* 只在登录大厅且未加入队列时执行 */
        if (isInLoginLobby() && !hasJoinedQueue && !waitingForQueueUse) {
            /* 容器内点击指南针 */
            ...
            if (compass.isHotbar() && queueTimer.passedS(joinQueueDelay.get())) {
                InvUtils.swap(compass.slot(), false);
                mc.interactionManager.interactItem(mc.player, Hand.MAIN_HAND);
                queueTimer.reset();
                waitingForQueueUse = true;
                queueUseTimer.reset();
            }
        }

        /* 等待右键生效后标记为已加入队列 */
        if (waitingForQueueUse && queueUseTimer.passedS(queueUseDelay.get())) {
            hasJoinedQueue = true;
            waitingForQueueUse = false;
        }
    }
